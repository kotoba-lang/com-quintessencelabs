(ns quintessencelabs.main
  "Kotodama WASM entrypoint for the Quintessencelabs clean-room actor (L4) — Clojure port.
  ...
  facts (`quintessencelabs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "quintessencelabs")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"quintessencelabs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "quintessencelabs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :quintessencelabs-compat/ready)
